...
async function hasRepeatings(filepath, mapActionsTextReg) {
  try {
    const fileContent = (await readFile(filepath)).replace(/\s+/g, " ");
    ...
    const actions = uniq(
      getActions(removeInnerCurlyBracesContent(mapActionsText))
    ).sort();

    const withOutMapText = fileContent.replace(mapActionRegexp, "");
    const unusedAction = actions.filter(
      action => !isInText(withOutMapText, action)
    );
    return {
      filepath,
      hasActions,
...
async function doWork(projectPath, unusedInstanceName, reg) {
  try {
    ...
    const repeatings = await Promise.all(
      vueFiles.map(file => hasRepeatings(file, reg))
    );
    ...
    const result = withUnusedActions
      ...
      .join("\n");
    const amountOf = withUnusedActions
      .map(e => e.unusedAction.length)
      .reduce((x, y) => x + y, 0);
    console.log(result);
...
function findNotUsed(fileContent) {
  ...
  try {
    ...
    const propertiesNames = propertiesNodes
      .map(R.pathOr("", ["key", "name"]))
      .filter(e => e !== "");
...
async function findNotUsedDataElements(path) {
  try {
    ...
    const fileContents = (await Promise.all(
      vueFiles.map(e => readFile(e))
    )).map(e => e.toString());
    const analyzedFiles = await Promise.all(
      fileContents.map((cont, ind) => analyzeDataElements(cont, vueFiles[ind]))
    );
...
async function doAllWork(path) {
  ...
  await doWork(path, "actions", /\.\.\.mapActions\({.*?}\)/gm);
  await doWork(path, "map states", /\.\.\.mapState\({.*?}\)/gm);
  await doWork(path, "getters", /\.\.\.mapGetters\({.*?}\)/gm);
}
...
if (process.argv.length < 3) {
  try {
    ...
    if (R.isNil(obj.projectPath))
      throw "Wrong JSON, need 'projectPath' property";
    doAllWork(obj.projectPath);
  } catch (err) {
    console.error(
      'Cannot find `not-used-config.json` file with "projectPath" property'
    );
    console.error(err);
  }
} else {
